...
void DF1_q12(q12 *x, q12 *y, q12 *b, q12 *a, int N, int order) {
    for (int n = 0; n < N; ++n) {
        ...
        for (int i = 0; i < order; ++i) {
            if (n - i >= 0) 
                acc = q12_add(acc, q12_mul(b[i], x[n - i]));
            if (i > 0 && n - i >= 0) 
                acc = q12_sub(acc, q12_mul(a[i], y[n - i]));
        }
...
void DF2_q12(q12 *x, q12 *y, q12 *b, q12 *a, int N, int order) {
    ...
    for (int n = 0; n < N; ++n) {
        ...
        for (int i = 1; i < order; ++i) {
            w[0] = q12_sub(w[0], q12_mul(a[i], w[i]));
        }

        int32_t acc = 0;
        for (int i = 0; i < order; ++i) {
            acc = q12_add(acc, q12_mul(b[i], w[i]));
        }
...
void TDF2_q12(q12 *x, q12 *y, q12 *b, q12 *a, int N, int order) {
    ...
    for (int n = 0; n < N; ++n) {
        int32_t acc = q12_add(w[0], q12_mul(b[0], x[n]));

        for (int i = 0; i < order - 2; ++i) {
            w[i] = q12_add(
                        q12_sub(w[i + 1], q12_mul(a[i + 1], acc)),
                        q12_mul(b[i + 1], x[n])
                    );
        }

        w[order - 2] = q12_sub(q12_mul(b[order - 1], x[n]),
                               q12_mul(a[order - 1], acc));
...
void CASCADE_q12(q12 *x, q12 *y, q12 *sos, int N, int sections) {
    ...
    for (int s = 0; s < sections; ++s) {
        ...
        for (int n = 0; n < N; ++n) {
            int32_t wn = q12_sub(in[n],
                               q12_add(q12_mul(a1, w1), q12_mul(a2, w2)));

            out[n] = q12_add(
                         q12_add(q12_mul(b0, wn), q12_mul(b1, w1)),
                         q12_mul(b2, w2)
                     );

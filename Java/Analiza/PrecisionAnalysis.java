...
public class PrecisionAnalysis {
    ...
    static void Precision(FileWriter fp, String filterName, String structure,
        ...
        writeErrorVector(fp, filterType, "float", structure, cutoff, order - 1, "impulse", yD, yF);

        // Szum
        generateWhiteNoise(xD);            // generujemy tylko w double
        for (int i = 0; i < N; i++) {
            xF[i] = (float) xD[i];         // konwersja na float
        }
        funcF.apply(xF, yF, bF, aF, N, order);
        funcD.apply(xD, yD, bD, aD, N, order);
        writeErrorVector(fp, filterType, "float", structure, cutoff, order - 1, "rand", yD, yF);
    }

    // === Analiza precyzji (SOS) ===
    static void PrecisionCascade(FileWriter fp, String filterName,
        ...
        writeErrorVector(fp, filterType, "float", "CASCADE", cutoff, 2 * sections, "impulse", yD, yF);
        ...
        funcF.apply(xF, yF, flatF, N, sections);
        funcD.apply(xD, yD, flatD, N, sections);
        writeErrorVector(fp, filterType, "float", "CASCADE", cutoff, 2 * sections, "rand", yD, yF);
    }

    // === Error Vector zapis do CSV ===
    private static void writeErrorVector(FileWriter fp, String filterType, String dtype, String structure,
                                         int cutoff, int order, String signal,
                                         double[] yRef, float[] yTest) throws IOException {
        fp.write(String.format(Locale.US, "%s,%s,%s,%d,%d,%s,", filterType, dtype, structure, cutoff, order, signal));

        for (int i = 0; i < yRef.length; i++) {
            double err = yRef[i] - yTest[i];
            fp.write(String.format(Locale.US, "%.8e", err));
            if (i < yRef.length - 1) {
                fp.write(";");
            }
        }
    ...
    public static void main(String[] args) {
        try (FileWriter fp = new FileWriter("java_precision_results.csv")) {
            fp.write("filter_name,type,structure,cutoff,order,signal,error_vector\n");

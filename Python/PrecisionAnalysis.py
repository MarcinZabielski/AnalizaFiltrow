...
def main():
    ...
    impulse[0] = 1
    random_signal = np.random.uniform(low=-1.0, high=1.0, size=100)
    ...
    # Zapisz do CSV
    results_file = "filter_precision_results.csv"
    with open(results_file, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['filter_type', 'order', 'cutoff', 'structure', 'signal', 'dtype', 'MAE'])
                                ...
                                try:
                                    ...
                                    for dtype in dtypes:
                                        ...
                                        try:
                                            ...
                                            mae = np.mean(np.abs(error).astype(np.float64))
                                            writer.writerow([ftype, order, cutoff, struct_name, signal_name, dtype.__name__, mae])

                                        except Exception as e:
                                            print(f" Error [{ftype}-{order}-{cutoff}Hz-{struct_name}-{dtype}]: {e}")
                                            writer.writerow([ftype, order, cutoff, struct_name, signal_name, dtype.__name__, 'ERROR', 'ERROR'])
